...
def AUS_active_permits():
    ...
    count = client.get(data_set, where="permittype='BP' AND work_class='New' AND status_current='Active'", select="COUNT(*)")
    ...
    while True:
        ...
        results.extend(client.get(data_set, where="permittype='BP' AND work_class='New' AND status_current='Active'", select="permit_type_desc,permit_class_mapped,issue_date,issued_in_last_30_days,status_current,expiresdate,original_address1,original_city,original_state,original_zip,link,project_id,latitude,longitude", offset=start, limit=chunk_size))
...
def AUS_closed_permits():
    ...
    while True:
        # First 50000 results, returned as JSON from API / converted to Python list of dictionaries by sodapy.
        results.extend(client.get(data_set, where="permittype='BP' AND work_class='New' AND status_current='Final'", select="permit_type_desc,permit_class_mapped,issue_date,issued_in_last_30_days,status_current,expiresdate,original_address1,original_city,original_state,original_zip,link,project_id,latitude,longitude", offset=start, limit=chunk_size))
        # Shift query to next chunk
        start = start + chunk_size
        # If all records fetched, end loop
        if (start > int(count[0]['COUNT'])):
