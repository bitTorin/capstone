...
Congratulations, you have successfully completed the initial installation of the application. The following steps are optional but will allow us to flesh out the site to show more information regarding specific buildings we want to highlight in the city.  
...
Once saved, we will also need to link to this building via the appropriate permit model instance. Navigate to the Permits admin and search for `2020-071590 BP` as defined in the entry above. Now click on the `id` for that entry and edit the permit to save `Block 185` in the `Building` field.

Now when you navigate to `127.0.0.1:8000/city/Austin` the permit associated with the building will appear red. Hovering over the permit shows the building's name and clicking on the pin will display a sidebar containing all of that building's info.  

### Step 7: Add headline

The last step is to add some news articles so that they will appear within the building's infobar. Create a new headline via the Django admin interface. For this tutorial, you can enter the following:
	>Datetime: 2021-07-22
	>	  			 14:13:30
...
Once saved, you can reload the website to `127.0.0.1:8000/city/Austin` and click on the red pin titled `Block 185`, the building sidebar will pop up and the article will be featured within the dropdown at the bottom.

Congratulations, you've completed the set up and can now add permits, buildings, and news articles to the web application.
